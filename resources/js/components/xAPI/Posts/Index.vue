<script>
    export default {
        ...
        methods: {
            ...
            fetchPosts() {
                axios.get('/api/xapiposts')
                    // Added extra `data` wrapper from Eloquent API Resource call: `PostResource`:`toArray` method
                    .then(response => {
                        // console.log('\n/api/xapiposts response: ', response.data);

                        // if request is successful, store posts
                        this.posts = response.data.data;
                    })
                    // if request has error, log error
            ...
            <table class="min-w-full divide-y divide-gray-200 border">
                <thead>
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-3 bg-gray-50 text-left">
                            <span class="text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Category ID</span>
                        </th>
...
<!--
    ...
    This component uses: Vue 2 Options API
